...
B_z^{(ext)} &\approx B(z) , \\
B_\theta^{(ext)} &= 0 (\partial \phi_m / \partial \theta = 0) .
\end{aligned}
\end{equation}
...
Two more steps. First, use $z$ as independent variable: $dz = \beta c dt$. Using $' \equiv d/dz$ and dropping the $\approx$ symbol gives:
...
Rearrange and switch to $z$ as independent variable. In the paraxial limit:
...
(i) inertial term; (ii) accelerative damping; $E_r$ from converging field lines; (iii) solenoidal focusing ($v_\theta B_z$, part of centrifugal term); (iv) centrifugal defocusing.


---

We can also account for linearized self-fields. Assuming cylindrical symmetry of the beam, the Poisson equation for the electric field is:
...
Including the linear self fields in the paraxial limit adds a driving term to the right hand side:
...
Multiply the Vlasov equation by $g(\mathbf{x}, \mathbf{x}')$, an arbitrary function of the phase space coordinates, and integrate over all phase space space:
