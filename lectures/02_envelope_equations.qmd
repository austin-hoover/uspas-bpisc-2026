...
Repeat the process in Cartesian coordinates. Envelope equations:
...
\begin{align}
\frac{d(\gamma m \dot{x})}{dt} &= q \left( E_x + \dot{y} B_z - \dot{z} B_y \right) , \\
\frac{d(\gamma m \dot{y})}{dt} &= q \left( E_y + \dot{z} B_x - \dot{x} B_z \right) . \\
\end{align}
