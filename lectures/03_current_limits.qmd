...
Let $\kappa_x(s) = f(s) \kappa_0$ and $\kappa_y(s) = -$\kappa_x(s)$. Ignoring acceleration ($\gamma' = 0$):
...
f(s) =
\begin{cases}
  +1 & \text{if } 0               \leq s / 1 < \eta / 2 \\
  -1 & \text{if } 1 - \eta L / 2  \leq s / L < 1 + \eta / 2 \\
  +1 & \text{if } 2 - \eta / 2 \leq s / L < 2
\end{cases}
\end{equation}
...
\begin{align}
-\left[ 
  \delta \frac{\pi^2}{L^2} - \frac{4 \kappa_0}{\pi} \sin\left(\frac{\eta \pi}{2}\right) 
\right] 
\cos\left(\frac{\pi s}{L}\right)
+ \frac{2 \kappa_0 \delta}{\pi} \sin\left(\frac{\eta \pi}{2}\right)
&= 
...
\begin{align}
\delta &= \frac{4 \kappa_0 L^2}{\pi^3} \sin\left(\frac{\eta\pi}{2}\right)
\\
Q &= \frac{4 \kappa_0 \delta}{\pi} \sin\left(\frac{\eta\pi}{2}\right) \tilde{r}_{0}^2
\end{align}


\begin{equation}
\boxed{
  Q_{max} \approx \frac{2 \kappa_0 L \eta}{\pi} \left[ \frac{\sin(\eta\pi / 2)}{(\eta\pi / 2)} \right]^2 \tilde{r}_0^2
}
\end{equation}
