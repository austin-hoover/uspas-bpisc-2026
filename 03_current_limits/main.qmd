...
![](images/fodo.png){fig-align=center height=175px}
...
\begin{equation}
Q_{max} = 2 \omega_{0}^2 \tilde{r}_{0}^2
\end{equation}

Comparing to Eq. \eqref{eq:qmax_fodo} gives the oscillation frequency $\omega_0^2$ for the smooth-focusing approximation of the FODO channel:

\begin{equation}
\omega_0^2 = \frac{2 \kappa^2 L^2 \eta^2}{\pi^2}\left( \frac{\sin(\eta\pi / 2)}{(\eta \pi / 2)} \right)^2
\end{equation}


---

Define the phase advance $\sigma_0 = \omega_0 / (2 L)$ to eliminate $L$:

\begin{equation}
\omega_0^2 = \frac{\eta k \sigma_0}{\sqrt{2} \pi} \frac{\sin(\eta\pi / 2)}{(\eta \pi / 2)}
\end{equation}

\begin{equation}
Q_{max} 
= 2 \omega_{0}^2 \tilde{r}_{0}^2
...
![](images/summary_perveance.png){fig-align=center width=400px}

For low-energy beams ($\gamma \approx 1$), $\lambda_{max} \sim Q_{max} V$ where $V = \frac{1}{2} m \beta^2 c^2 / q$. 

![](images/summary_line_density.png){fig-align=center width=400px}
