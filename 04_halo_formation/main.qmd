...
Define \textit{depressed} focusing strength $k^2 \equiv k_0^2 - {Q} / {r_0^2}$:
...
Substituting \eqref{eq-env-deviations} into \eqref{eq-env_continuous} gives:
...
\eqref{eq-env-pert-coupled-1} defines a coupled linear system of equations for $\eta_{x, y}$. We will analyze this system in terms of its normal modes. In vector form, @eq-env-pert-coupeld-1 can be written:
...
where $ks$ is the unperturbed phase advance within the beam and $\alpha(s)$ is a slowly varying function. We will derive dynamical equations for $A$ and $\psi$ by substituting @eq-pcm-phase-amp-x and @eq-pcm-phase-amp-xp into @eq-pcm-eom-pendulum. 


---

First, note that the $s$-derivative of @eq-pcm-phase-amp-x gives
